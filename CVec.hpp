...
concept IsChar = ::std::same_as<::std::remove_cv_t<CharT>, char> || ::std::same_as<::std::remove_cv_t<CharT>, wchar_t> || ::std::same_as<::std::remove_cv_t<CharT>, char8_t> || ::std::same_as<::std::remove_cv_t<CharT>, char16_t> || ::std::same_as<::std::remove_cv_t<CharT>, char32_t>;
...
struct CVec {
    ...
    constexpr const value_type* end() const { return &data[switchValue<IsChar<::std::remove_cv_t<value_type>>, L - 1, L>]; }
};
...
constexpr bool operator==(const CVec<T, L1>& a, const CVec<U, L2>& b) {
    ...
    else if (::std::convertible_to<::std::remove_cv_t<U>, ::std::remove_cv_t<T>> && !::std::same_as<::std::remove_cv_t<U>, ::std::remove_cv_t<T>>) {
        auto t = static_cast<CVec<::std::remove_cv_t<T>, L1>>(b);
        for (::std::size_t i = 0; i < L1; i++)
            if (a[i] != t[i]) return false;
    } else if (::std::convertible_to<::std::remove_cv_t<T>, ::std::remove_cv_t<U>> && !::std::same_as<::std::remove_cv_t<T>, ::std::remove_cv_t<U>>) {
...
template <typename T, ::std::size_t L1, typename U, ::std::size_t L2>
constexpr CVec<T, switchValue<IsChar<::std::remove_cv_t<T>> || IsChar<::std::remove_cv_t<U>>, L1 + L2 - 1, L1 + L2>> operator+(const CVec<T, L1>& a, const CVec<U, L2>& b) {
    CVec<T, switchValue<IsChar<::std::remove_cv_t<T>> || IsChar<::std::remove_cv_t<U>>, L1 + L2 - 1, L1 + L2>> res;
    for (::std::size_t i = 0; i < switchValue < IsChar<::std::remove_cv_t<T>> || IsChar<::std::remove_cv_t<U>>, L1 - 1, L1 > ; i++) res[i] = a[i];
    for (::std::size_t i = 0; i < L2; i++) res[i + switchValue < IsChar<::std::remove_cv_t<T>> || IsChar<::std::remove_cv_t<U>>, L1 - 1, L1 > ] = b[i];
    return res;
}
