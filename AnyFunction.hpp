...
constexpr auto any_call_base(auto &fn, CTDArray<Any> &args, auto &...pack)
    -> Any {
  if constexpr (I < Argc) {
...
template <typename T> constexpr auto any_call(T &fn, CTDArray<Any> &args) {
...
struct AnyFunction {
  ...
  template <typename T> constexpr AnyFunction(const T &fn) : fn(fn) {
